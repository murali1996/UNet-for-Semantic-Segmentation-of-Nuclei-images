...
def get_bw_mask(mask_path, k_gap_erode=4):
    ...
    return bw_mask
#############################################################################################################
def thisPatch(image, bw_mask, curr_row, curr_col, base_name,
    ...
    if zoomIn and np.random.uniform(0,1)>=0.9:
...
def extract_patches_and_save(item, image, bw_mask):
    # Desired values
    ...
    while(curr_row+row_length<=img_rows):
        ...
        while(curr_col+col_length<=img_cols):
            ...
            new_image, new_mask, new_gen_name = thisPatch(image, bw_mask, curr_row, curr_col, base_name);
...
#############################################################################################################
def generate_train_data_with_augmentation(target_folder): # Rotations+Patches
...
def load_test_list():
    ...
    return ts_list
############################################################################################################
def generate_original_copies_train_data():
    org_names = [];
    for item in os.listdir(params.train_folder_org):
        org_names+=[item]; print(len(org_names));
    ...
    print('Training Data generated success...');
    return org_names
def generate_original_copies_test_data():
    org_names = [];
...
if __name__=="__main__":
    #generate_original_copies_train_data();
    #generate_original_copies_test_data();
    ...
    #tr_list = load_train_list_with_augmentation(target_folder);
    #ts_list = load_test_list()
